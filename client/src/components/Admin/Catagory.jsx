  ...
  const handleDeleteCatagory = async (catagoryId) => {
    try {
      const response = await fetch(
        `/api/catagory/deletecatagory/${catagoryId}`, {
        method: "DELETE",
    });
        ...
        <table className="  text-sky-900   border-separate border-spacing-y-2 w-[600px]">
          <tr className=" ">
            ...
            <td className=" text-center">
              <Link to="/create-catagory" className=" border  text-blue-800 hover:bg-purple-500 hover:text-white border-blue-800 px-4 py-1 mr-1 font-semibold">
                Add <ArrowRight />
              </Link>
            </td>
          </tr>
